  ...
  try {
    ...
    return NextResponse.json({
      ...
      member: member[0]
        ? {
            id: member[0].id,
            individualPaid: Boolean(member[0].individual_training_paid),
            individualStartDate: member[0].individual_start_date,
            individualExpiryDate: member[0].individual_expiry_date,
          }
        : null,
    });
  } catch (error) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!startsAt || !endsAt) {
      return NextResponse.json(
        { error: "Unesite ispravan datum i vreme termina" },
        { status: 400 },
      );
    }
    ...
    if (new Date(startsAt).getTime() >= new Date(endsAt).getTime()) {
      return NextResponse.json(
        { error: "Kraj termina mora biti posle početka" },
        { status: 400 },
      );
    }
