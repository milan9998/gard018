...
export async function POST(request: Request) {
  ...
  if (adminAuth.isAdmin) {
    return NextResponse.json({ error: "Admin pregleda termine i rezervacije, ali ne rezerviše trening." }, { status: 403 })
  }
  ...
  try {
    ...
    const memberResult = await sql`
      SELECT id, individual_training_paid
      FROM members
    ...
    if (memberResult.length === 0) {
      return NextResponse.json({ error: "Vaš nalog nije povezan sa članstvom" }, { status: 403 })
    }

    const member = memberResult[0]
    if (!member.individual_training_paid) {
      return NextResponse.json({ error: "Individualni trening još nije plaćen" }, { status: 403 })
    }
    ...
    if (slotResult.length === 0 || slotResult[0].status !== "open") {
      return NextResponse.json({ error: "Termin nije dostupan" }, { status: 404 })
    }

    if (new Date(slotResult[0].starts_at).getTime() < Date.now()) {
      return NextResponse.json({ error: "Termin je već počeo ili je prošao" }, { status: 400 })
    }
    ...
    if (existing.length > 0) {
      return NextResponse.json({ error: "Već ste rezervisali ovaj termin" }, { status: 409 })
    }
    ...
    if (result.length === 0) {
      return NextResponse.json({ error: "Ovaj termin nije moguće ponovo rezervisati" }, { status: 409 })
    }

    return NextResponse.json({ booking: result[0] }, { status: 201 })
  } catch (error) {
    console.error("[GARD018] Individual booking creation failed:", error)
    return NextResponse.json({ error: "Greška pri rezervaciji termina" }, { status: 500 })
  }
}
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    if (!Number.isInteger(id)) return NextResponse.json({ error: "Nevažeća rezervacija" }, { status: 400 })
    ...
    if (result.length === 0) return NextResponse.json({ error: "Rezervacija nije pronađena" }, { status: 404 })
