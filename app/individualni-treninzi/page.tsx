...
type MemberAccess = {
  ...
  individualExpiryDate: string | null;
};
...
export default function IndividualniTreninziPage() {
  ...
  const loadSlots = async (showLoading = true) => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(data.error || "Greška pri učitavanju termina");
      setSlots(data.slots || []);
  ...
  if (checkingRole) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-background text-muted-foreground">
        <Loader2 className="mr-3 h-6 w-6 animate-spin" /> Provera pristupa...
      </div>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-background px-4 py-24">
      <div className="mx-auto max-w-4xl">
        ...
        {!requiresLogin &&
          memberAccess &&
          (!memberAccess.individualPaid ||
            !memberAccess.individualExpiryDate) && (
            <div className="mb-6 rounded-lg border border-yellow-500/30 bg-yellow-500/10 p-4 text-yellow-200">
              Admin još nije aktivirao vaš period individualnog treninga. Kada
              uplata bude evidentirana, ovde ćete moći da rezervišete termine.
            </div>
          )}
        ...
        {requiresLogin && (
          <div className="rounded-lg border border-primary/20 bg-card/40 p-8 text-center">
            ...
            <h2 className="text-2xl font-semibold text-foreground">
              Prijavite se da vidite termine
            </h2>
            <p className="mt-2 text-muted-foreground">
              Posle prijave sistem proverava vaše pravo na individualni trening.
            </p>
            <Link href="/prijava">
        ...
        {!requiresLogin && loading && (
          <div className="flex items-center justify-center py-16 text-muted-foreground">
            <Loader2 className="mr-3 h-6 w-6 animate-spin" /> Učitavanje
            termina...
          </div>
        )}
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {slots.map((slot) => {
              ...
              return (
                <div
                  key={slot.id}
                  className="rounded-lg border border-primary/20 bg-card/40 p-6"
                >
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-2 flex items-center gap-2 text-sm text-muted-foreground">
                        ...
                        Do{" "}
                        {new Date(slot.ends_at).toLocaleTimeString("sr-RS", {
                          hour: "2-digit",
                          minute: "2-digit",
                        })}
                      </p>
                    ...
                    <span className="text-sm text-muted-foreground">
                      {slot.booking_count} prijavljenih
                    </span>
                  </div>
